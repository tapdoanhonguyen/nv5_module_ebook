...
//  ---------------------------------------
function nv_link(author) {

    var nv_timer = nv_settimeout_disable('from_signer_' + author, 500);
    var new_status = document.getElementById('from_signer_' + author).options[document.getElementById('from_signer_' + author).selectedIndex].value;

    if (new_status != 0) {
        $('#hienthi').show();
        $.post(script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=add_book&nocache=' + new Date().getTime(), 'action=1&author=' + new_status + '&num=' + nv_randomPassword(8), function(res) {
            $('#hienthi').html( res );
        });
    }
}
